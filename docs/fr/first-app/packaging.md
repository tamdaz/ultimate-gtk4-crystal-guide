...
En suivant la section [Makefile](../extra/makefile), nous allons la créer avec le contenu suivant :
...
Même si cela semble un peu désordonné, elle couvre tout ce qui est nécessaire. Les dépendances
sont générées en utilisant le script qui est mentionné dans la section [Flatpak](../packaging/flatpak).
